...
import app.pankaj.api.auth.domain.model.Login
...
suspend fun ApplicationCall.isLoginIsValidRequest(): Pair<String, Login?> {
    val request = this.receiveNullable<Login>()
    var error = ""
    if (request == null) {
        error = "Invalid or missing request body."
        return Pair(error, null)
